...
class Tabs:
    ...
    def set_current_tab(self, tab: tabs.Tab) -> bool:
        """Set the current tab"""
        if (not isinstance(tab, tabs.GapTab) and
                0 <= tab.nb < len(self._tabs)):
            self._store_previous()
            self._current_index = tab.nb
            self._current_tab = tab
            return True
    ...
    def by_name_and_class(self, name: str, cls: Type[tabs.Tab]) -> Optional[tabs.Tab]:
    ...
    def _validate_current_index(self):
        if not 0 <= self._current_index < len(self._tabs) or not self.current_tab:
            self.prev()
    ...
    def _insert_gaps(self, old_pos: int, new_pos: int) -> bool:
        ...
        if not target:
            if new_pos < len(self._tabs):
                ...
                self._tabs[new_pos], self._tabs[old_pos] = old_tab, tabs.GapTab(
                    self)
            else:
    ...
    def insert_tab(self, old_pos: int, new_pos=99999, gaps=False) -> bool:
        ...
        """
        if (old_pos <= 0 or old_pos >= len(self._tabs) or new_pos <= 0 or
                new_pos == old_pos or not self._tabs[old_pos]):
            return False
        if gaps:
